...
def get_action_result_text(is_valid: bool, errors: list, answer: AnswerModel, added_items: list, updated_items: list, deleted_items: list) -> str:
    ...
    if is_valid and answer.response:
        ...
        if reply_parts:
            return f"🤖 Команды выполнены:\n\n{escape_md(chr(10).join(reply_parts))}"
    else:
        ...
        if answer and answer.error:
            error_text += f"\n\nОшибка LLM: {escape_md(answer.error)}"
        return error_text
    
    

@voice_router.message(F.voice)
async def handle_voice_message(message: types.Message, db_session: Session, db_user: User):
    """
    ...
    text_answer = send_prompt_to_llm(text, db_session)
